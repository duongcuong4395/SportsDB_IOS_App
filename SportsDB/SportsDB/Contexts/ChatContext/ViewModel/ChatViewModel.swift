...
class ChatViewModel: ObservableObject {
    ...
    func initChat() {
        if chat == nil {
            let modelKey = getKey()
            let model = getModel(with: modelKey, and: .gemini_2_0_flash_exp, and: nil)
...
extension ChatViewModel {
    func aiSend(prompt: String) async throws -> String {
        guard let chat = chat else { return "" }
        do {
            let response = try await chat.sendMessage(prompt)
            return response.text ?? ""
        } catch {
            print("==== aiSend.error", error.localizedDescription)
