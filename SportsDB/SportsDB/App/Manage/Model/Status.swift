...
class MyViewModel: ObservableObject {
    @Published var modelsBytatus: ModelsStatus<[Country]> = .idle
    @Published var models: [Country] = []
    
    func fetchModels() {
        Task {
            let res = try await CountryAPIService().getAllCountries()
            modelsBytatus = .success(data: res)
            models = res
        }
        
    }
    
    func updateModel(by oldCountry: Country, newCountry: Country) {
        // MARK: update for models
        guard let index = models.firstIndex(where: { $0.name == oldCountry.name }) else { return }
        models[index] = newCountry
        
        // MARK: update for modelsByStatus
        // how to update oldCountry by newCountry into modelsBytatus : ModelsStatus<[Country]> ???
    }
    
    func filterModels(by textSearch: String) {
        // MARK: filter for models
        models = models.filter({ $0.name.contains(textSearch) })
        
        // MARK: filter for modelsByStatus
        // how to filter into modelsBytatus : ModelsStatus<[Country]> ???
    }
    
    func deleteModel(by country: Country) {
        // MARK: delete a country into models
        models.removeAll(where: { $0.name == country.name })
        
        // MARK: delete a country into modelsByStatus
        // how to delete a country into modelsBytatus : ModelsStatus<[Country]> ???
    }
}
